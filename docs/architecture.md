...
кадр в кеше остаётся неизменным. Профиль запрашивается у colord по имени экрана,
а на X11 также читается из стандартного свойства корневого окна `_ICC_PROFILE`
(или `_ICC_PROFILE_n`). Сигнал
